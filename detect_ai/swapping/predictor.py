...
class _TimingLogger:
    """Utility for printing timing data when env flag is enabled."""

    def __init__(self) -> None:
        flag = os.environ.get("SWAPPING_TIMING", "1").strip().lower()
        self.enabled = flag not in {"0", "false", "off"}

    def log(self, label: str, elapsed: float) -> None:
        if self.enabled:
            print(f"[SwappingTiming] {label}: {elapsed:.3f}s")
...
class FaceCropper:
    def __init__(self, models_dir: str):
        ...
        if not os.path.exists(model_path):
            ...
            raise dai.ModelLoadingError(f"Отсутствует обязательный файл модели детекции лиц: {model_path}")
        ...
        try:
            ...
            raise dai.ModelLoadingError(f"Ошибка инициализации FaceDetectorYN: {e}")
...
class DeepfakePredictor:
    def __init__(self, models_dir: str):
        ...
        try:
            ...
            if not os.path.exists(fb_dir):
                raise dai.ModelLoadingError(f"Папка с feature-based моделями не найдена: {fb_dir}")

            with self._timings.measure("init:load_feature_models"):
            ...
            self.effnet_models = {}
            with self._timings.measure("init:load_effnet_models"):
            ...
            raise dai.ModelLoadingError(f"Не удалось загрузить модель: {e}")
    ...
    def _validate_features(self, df: pd.DataFrame, required_features: list, model_name: str):
        ...
        if missing_features:
            ...
            raise dai.FeatureExtractionError(
    ...
    def _get_feature_based_probs(self, df: pd.DataFrame) -> dict:
        try:
            ...
            X_fl_expanded = dai.flatten_landmarks(df_landmarks)
            ...
            raise dai.FeatureExtractionError(f"Ошибка доступа к признакам при прогнозировании: {e}")
        except Exception as e:
            raise dai.SwappingError(f"Ошибка при расчете вероятностей на основе признаков: {e}")

    def predict(self, image_path: str, debug: bool = False) -> dict:
        total_start = time.perf_counter()

        with self._timings.measure("run:calculate_features"):
            features_df, timing_stats = dai.calculate_features_one(image_path)
        ...
        try:
            ...
            with self._timings.measure("run:effnet"):
                with torch.no_grad():
        ...
        try:
            with self._timings.measure("run:meta_classifier"):
            ...
            raise dai.SwappingError(f"Ошибка при формировании финального решения: {e}")
        finally:
