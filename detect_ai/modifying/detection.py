...
class ModDetector:
    ...
    def _get_model(self):
        ...
        return dai.ModClassifier(model_path=self.model_path, class_indices_path=class_indices_path)
    ...
    def _load_lbp_features(self):
        with open(f'{os.path.dirname(self.model_path)}/lbp.json', 'r') as f:
            params = json.load(f)
        params = {key: val for key, val in params.items() if key in dai.extract_lbp_features.__code__.co_varnames}
        return 'lbp', params
    ...
    def _load_hog_features(self):
        ...
        params = {key: val for key, val in params.items()
                  if (key in dai.extract_hog_features.__code__.co_varnames) or (key == 'input_size')}
    ...
    def _load_face_landmarks_features(self):
        with open(f'{os.path.dirname(self.model_path)}/face_landmarks.json', 'r') as f:
            params = json.load(f)
        params = {key: val for key, val in params.items() if key in dai.extract_face_landmarks.__code__.co_varnames}
    ...
    def prepare_data(self, img_path: str) -> np.ndarray | None:
        ...
        if self.feature_type == 'face_landmarks':
            with self._timings.measure("prepare:face_landmarks"):
                data = dai.load_img(img_path, as_mediapipe_image=True)
                data = dai.extract_face_landmarks(data, **self.feature_params)
            if data is None:
                return None

        elif self.feature_type == 'lbp':
            with self._timings.measure("prepare:lbp"):
                data = dai.load_img(img_path, as_gray=True)
                if (self.feature_params.get('histogram') in [None, False]) and (data.shape != input_shape):
                    data = dai.resize_img(data, input_shape)
                data = dai.extract_lbp_features(data, **self.feature_params)
            ...
            with self._timings.measure("prepare:plain"):
                data = dai.load_img(img_path, as_gray=False)
                if data.shape != input_shape:
                    data = dai.resize_img(data, input_shape[:-1])
