...
def calculate_features_one(image_path: str) -> tuple:
    ...
    if image is None:
        ...
        raise dai.FeatureExtractionError(f"Не удалось прочитать изображение: {image_path}")
    ...
    with mp_face_mesh.FaceMesh(
        ...
        if not results.multi_face_landmarks:
            ...
            raise dai.FeatureExtractionError(f"MediaPipe не нашел лицо на изображении: {image_path}")
    ...
    feat_extr = dai.FaceFeatureExtractor()
    try:
        ...
        raise dai.FeatureExtractionError(f"Ошибка при извлечении структурных признаков лица: {e}")
...
def flatten_landmarks(df_landmarks: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(sample_cell, str):
        try:
            ...
            raise dai.FeatureExtractionError(f"Ошибка парсинга строковых ориентиров: {e}")
    else:
