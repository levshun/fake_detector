...
DEFAULT_GENERATING_DIR = ROOT_DIR / "models" / "generating"
DEFAULT_GENERATING_VIT = DEFAULT_GENERATING_DIR / "eva_model.pth"
DEFAULT_GENERATING_CONVNEXT = DEFAULT_GENERATING_DIR / "convnext_model.pth"
DEFAULT_GENERATING_CLASSIFIER = DEFAULT_GENERATING_DIR / "final_decisiontree.pkl"
DEFAULT_MODIFYING_DIR = ROOT_DIR / "models" / "modifying"
DEFAULT_MODIFYING_MODEL = DEFAULT_MODIFYING_DIR / "binary" / "beauty_gan" / "eff_net_b3" / "eff_net_b3.keras"
...
TABL_CSV_PATH = ROOT_DIR / "tabl.csv"
...
def parse_args() -> argparse.Namespace:
    ...
    swap_group.add_argument(
        "--swapping-models-dir",
        type=Path,
        default=ROOT_DIR / "models" / "swapping",
        help="Каталог с моделями модуля swapping (по умолчанию 'models/swapping')",
    )
    swap_group.add_argument(
        "--skip-swapping",
        action="store_true",
...
def _resolve_generating_paths(args: argparse.Namespace) -> tuple[Path | None, Path | None, Path | None]:
    ...
    if vit is None and DEFAULT_GENERATING_VIT.exists():
        vit = DEFAULT_GENERATING_VIT
    if convnext is None and DEFAULT_GENERATING_CONVNEXT.exists():
        convnext = DEFAULT_GENERATING_CONVNEXT
    if clf is None and DEFAULT_GENERATING_CLASSIFIER.exists():
...
def _resolve_modifying_paths(args: argparse.Namespace) -> tuple[Path | None, Path | None]:
    ...
    if model is None and DEFAULT_MODIFYING_MODEL.exists():
...
def _get_modifying_detector(
    ...
    if ensemble_path:
        ...
        if key not in _MOD_CACHE:
            ...
            try:
                _MOD_CACHE[key] = start_ensemble(str(resolved_path))
            finally:
    ...
    if key not in _MOD_CACHE:
        start = time.perf_counter()
        _MOD_CACHE[key] = start_model(str(model_path))
        _INIT_TIMINGS["Modifying"] += time.perf_counter() - start
    return _MOD_CACHE[key]
...
def _get_swapping_predictor(models_dir: Path) -> DeepfakePredictor:
    key = str(models_dir.resolve())
    if key not in _SWAP_CACHE:
        start = time.perf_counter()
        _SWAP_CACHE[key] = DeepfakePredictor(models_dir=key)
...
def _warm_up(args: argparse.Namespace) -> None:
    ...
    if not args.skip_swapping and args.swapping_models_dir.is_dir():
